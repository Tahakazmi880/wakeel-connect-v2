...
export default function VideoRoom({ bookingId }: { bookingId: string }) {
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-10">
      ...
      <div
        role="status"
        className="rounded-3xl border-2 border-amber-300 bg-amber-50 p-6 text-center shadow-sm sm:p-8"
      >
        <span className="mx-auto flex h-16 w-16 items-center justify-center rounded-full bg-amber-100 text-3xl" aria-hidden>
          📞
        </span>
        <h1 className="mt-4 text-2xl font-extrabold text-amber-950 sm:text-3xl">
          <T en="Video calls aren't live in the app yet" ur="ویڈیو کال ابھی ایپ میں فعال نہیں" />
        </h1>
        <p className="mx-auto mt-3 max-w-lg text-lg text-amber-900">
          <T
      ...
      <div className="mt-6 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
        {!user ? (
          ...
          <>
            ...
            <h2 className="mt-1 text-2xl font-extrabold text-slate-900">{booking.lawyer.displayName}</h2>
            <dl className="mt-4 space-y-3 text-lg">
              <div className="flex items-center gap-3">
                <span aria-hidden className="text-2xl">🕐</span>
                <div>
                  <dt className="sr-only"><T en="Time" ur="وقت" /></dt>
                  <dd className="font-bold text-slate-800">{fmtDateTime(booking.startAt)}</dd>
                </div>
              </div>
              <div className="flex items-center gap-3">
                <span aria-hidden className="text-2xl">📋</span>
                <div>
                  ...
                  <dd className="font-bold text-slate-800"><T en={modeLabel(booking.mode).en} ur={modeLabel(booking.mode).ur} /></dd>
                </div>
              </div>
              <div className="flex items-center gap-3">
                <span aria-hidden className="text-2xl">💰</span>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <dd className="font-bold text-slate-800">{booking.status}</dd>
                </div>
              </div>
            </dl>
            {booking.mode === "ONLINE_VIDEO" && (
              <p className="mt-4 rounded-2xl bg-emerald-50 p-4 text-base font-semibold text-emerald-900 ring-1 ring-emerald-200">
                <T
        ...
        <div className="mt-6 flex flex-col gap-3 sm:flex-row">
          ...
          <Link href="/" className="inline-flex min-h-[52px] items-center justify-center rounded-2xl px-6 py-3 text-lg font-bold text-slate-600 hover:text-emerald-700 hover:underline">
