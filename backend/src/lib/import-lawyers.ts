// Import of the firm's 21 real lawyer profiles into PostgreSQL.
// Shared by the local tsx script (prisma/import-real-lawyers.ts) and the
// production bootstrapper (src/lib/bootstrap.ts).
//
// Source data: backend/src/lib/real-lawyers.json — extracted from
// frontend/lib/data.ts (the firm's 21 real, non-demo profiles).
// Idempotent: safe to re-run (upserts + wipe/recreate relations).
//
...
import { REAL_LAWYERS_DATA, type RealLawyerEntry } from "./real-lawyers-data.js";

const LAWYERS: RealLawyerEntry[] = REAL_LAWYERS_DATA;
...
export async function importRealLawyers(prisma: PrismaClient): Promise<void> {
  console.log(`Importing ${LAWYERS.length} real lawyers...`);
  ...
  for (const [i, l] of LAWYERS.entries()) {
